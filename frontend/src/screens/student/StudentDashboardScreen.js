    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      <View style={styles.header}>
        ...
        <View style={[styles.statusBadge, {
          backgroundColor: student.accountStatus === 'Active' ? COLORS.greenBg : COLORS.redBg,
        }]}>
          <Text style={[styles.statusText, {
            color: student.accountStatus === 'Active' ? COLORS.green : COLORS.red,
          }]}>{student.accountStatus}</Text>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.reminderCard}>
          <View style={styles.flex1}>
            ...
            <Text style={styles.reminderSubtext}>Get notified before your sessions</Text>
          </View>
          <Switch
            value={student.reminderNotifications}
            onValueChange={handleToggleReminders}
            trackColor={{ false: COLORS.borderLight, true: COLORS.brandOrange }}
            thumbColor={student.reminderNotifications ? COLORS.white : COLORS.textMuted}
          />
        </View>
        ...
        {student.enrolledCourses?.length === 0 ? (
          ...
          student.enrolledCourses?.map((course) => (
            <View key={course._id} style={styles.courseCard}>
              <Text style={styles.courseCategory}>{course.licenseCategory?.licenseCategoryName}</Text>
              <View style={styles.courseRow}>
                <View>
                  <Text style={styles.courseFee}>LKR {course.courseFee?.toLocaleString()}</Text>
                  {course.discount > 0 && (
                    <Text style={styles.courseDiscount}>Discount: LKR {course.discount?.toLocaleString()}</Text>
                  )}
                </View>
          ...
          student.bookedSessions?.map((session) => (
            <View key={session._id} style={styles.sessionCard}>
              ...
              <View style={[styles.badge, { backgroundColor: COLORS.blueBg }]}>
                <Text style={[styles.badgeText, { color: COLORS.blue }]}>{session.status}</Text>
              </View>
            </View>
          ))
        )}
...
const styles = StyleSheet.create({
  ...
  profileCard:  { alignItems: 'center', backgroundColor: COLORS.brandYellow, borderRadius: 20, padding: 20, marginBottom: 16, gap: 4 },
  avatar:       { width: 64, height: 64, borderRadius: 32, backgroundColor: 'rgba(255,255,255,0.6)', alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
  ...
  reminderSubtext:{ fontSize: 12, color: COLORS.textMuted },
  ...
  bookBtn:      { backgroundColor: COLORS.brandOrange, borderRadius: 12, paddingVertical: 12, alignItems: 'center', marginBottom: 12 },
  bookBtnText:  { color: COLORS.white, fontWeight: '600', fontSize: 14 },
  sessionCard:  { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.white, borderWidth: 1, borderColor: COLORS.border, borderRadius: 14, padding: 14, marginBottom: 10 },

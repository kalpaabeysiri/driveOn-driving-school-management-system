...
export default function StudentListScreen({ navigation }) {
  ...
  const handleDelete = (student) => {
    Alert.alert('Delete Student', `Delete ${student.firstName} ${student.lastName}? This cannot be undone.`, [
      { text: 'Cancel', style: 'cancel' },
      {
    ...
    >
      ...
      <View style={styles.actions}>
        <View style={[styles.badge, {
          backgroundColor: item.accountStatus === 'Active' ? COLORS.greenBg : COLORS.redBg,
        }]}>
          <Text style={[styles.badgeText, {
            color: item.accountStatus === 'Active' ? COLORS.green : COLORS.red,
          }]}>{item.accountStatus}</Text>
        </View>
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerBtns}>
          <TouchableOpacity style={styles.reportBtn} onPress={() => navigation.navigate('MonthlyReport')}>
...
const styles = StyleSheet.create({
  ...
  filterBtn:   { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20, backgroundColor: COLORS.bgLight },

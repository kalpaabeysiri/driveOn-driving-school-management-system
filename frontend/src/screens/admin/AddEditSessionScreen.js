...
import { createSession, updateSession, getSessionById } from '../../services/sessionApi';
import { getAllInstructors, getAllVehicles } from '../../services/instructorVehicleApi';
...
const [form, setForm] = useState({
  ...
  const validate = () => {
    if (!form.instructor) {
      Alert.alert('Error', 'Please select an instructor'); return false;
    }
    if (form.sessionType === 'Practical' && !form.vehicle) {
      Alert.alert('Error', 'Vehicle is required for Practical sessions'); return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        Alert.alert('Success', 'Session updated!', [{ text: 'OK', onPress: () => navigation.goBack() }]);
      } else {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{isEdit ? 'Edit Session' : 'Create Session'}</Text>
        <View style={{ width: 24 }} />
      </View>
        ...
        >
          ...
          <View style={styles.optRow}>
            {['Theory', 'Practical'].map((t) => (
              <TouchableOpacity
                key={t}
                style={[styles.optBtn, form.sessionType === t && styles.optBtnActive]}
                onPress={() => handleChange('sessionType', t)}
              >
                <Ionicons
                  name={t === 'Theory' ? 'book-outline' : 'car-outline'}
                  size={18}
                  color={form.sessionType === t ? COLORS.black : COLORS.textMuted}
                />
                <Text style={[styles.optText, form.sessionType === t && styles.optTextActive]}>{t}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          >
            ...
            <Text style={styles.pickerText}>{formatTime(selectedStartTime)}</Text>
            <Ionicons name="chevron-down" size={18} color={COLORS.textMuted} />
          </TouchableOpacity>
          ...
          {instructors.length === 0
            ? <Text style={styles.noDataText}>No instructors found. Add instructors first.</Text>
            : instructors.map((ins) => (
              <TouchableOpacity
                key={ins._id}
                style={[styles.selectCard, form.instructor === ins._id && styles.selectCardActive]}
              ...
              >
                ...
                <View style={styles.flex1}>
                  <Text style={styles.selectName}>{ins.fullName || ins.name}</Text>
                  <Text style={styles.selectMeta}>{ins.specialization} · {ins.contactNumber || ins.phone}</Text>
                </View>
          ...
          {form.sessionType === 'Practical' && (
            <>
              ...
              {vehicles.length === 0
                ? <Text style={styles.noDataText}>No available vehicles found.</Text>
                : vehicles.map((v) => (
                  <TouchableOpacity
                    key={v._id}
                    style={[styles.selectCard, form.vehicle === v._id && styles.selectCardActive]}
                  ...
                  >
                    ...
                    <View style={styles.flex1}>
                      <Text style={styles.selectName}>{v.brand} {v.model} ({v.year})</Text>
                      <Text style={styles.selectMeta}>{v.licensePlate} · {v.transmission}</Text>
                    </View>
                    {form.vehicle === v._id && (
                      <Ionicons name="checkmark-circle" size={22} color={COLORS.green} />
                    )}
          ...
          >
            {submitting
              ...
              : <Text style={styles.submitBtnText}>{isEdit ? 'Update Session' : 'Create Session'}</Text>
            }
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, marginBottom: 16,
  },
  optRow:        { flexDirection: 'row', gap: 12, marginBottom: 16 },
  optBtn: {
    flex: 1, borderRadius: 12, paddingVertical: 14, alignItems: 'center',
    gap: 6, backgroundColor: COLORS.bgLight, borderWidth: 1, borderColor: COLORS.border,
  },
  optBtnActive:  { backgroundColor: COLORS.brandYellow, borderColor: COLORS.brandYellow },
  optText:       { fontSize: 14, fontWeight: '600', color: COLORS.textMuted },
  ...
  pickerText:    { flex: 1, fontSize: 15, fontWeight: '600', color: COLORS.black },
  iosPickerWrap: {
  ...
  iosPickerHeader: {
    ...
    backgroundColor: COLORS.gray, paddingHorizontal: 16,
    paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: COLORS.border,
  },
  iosDoneBtn:    { backgroundColor: COLORS.brandOrange, borderRadius: 8, paddingHorizontal: 16, paddingVertical: 6 },
  iosDoneText:   { color: COLORS.white, fontWeight: '700', fontSize: 14 },
  ...
  noDataText:    { fontSize: 13, color: COLORS.textMuted, marginBottom: 16 },
